...
def DeepConvNetTrain(activation_function, opt, dropout):
    train_acc = []
    valid_acc = []
    
    net = DeepConvNet(activation_function, dropout).to(device)
    #print(net)
    ...
    for epoch in tqdm(range(EPOCH)):
        ...
        for idx, (inputs, labels) in enumerate(train_loader):
            inputs = inputs.type(TensorInputType).to(device)
            labels = labels.type(TensorLabelType).to(device)
            y_pred = net(inputs)
...
for drop in dropout:
    for act_func in activation_function:
        for optimizer in opts:
            print('===============\n\n')
            #csvName = 'test_csv\\DeepConvNet_' + act_func + '_' + str(drop) + '_' + optimizer + '_' + loss_function + '_' +'.csv'
            csvName = 'csv\\DeepConvNet_' + act_func + '_' + str(drop) + '_' + optimizer + '_' + loss_function + '_' +'.csv'
            print('Training...')
            print(csvName.split('\\')[1].split('csv')[0][:-2])
            train_acc, valid_acc = DeepConvNetTrain(activation_function=act_func, opt=optimizer, dropout=drop)
