...
class statistic:
    ...
    def show(self, tstat = True):
        ...
        """
        blk = min(len(self.data), self.block) # Blocks Count
        stat = [0] * 64
        sop, pop, eop = 0, 0, 0
        sdu, pdu, edu = 0, 0, 0
        ssc, msc = 0, 0
        for i in range(1, blk + 1):
            ep = self.data[-i]
            ssc += ep.score() # Current Score
            msc = max(ep.score(), msc) # Max Score
        ...
        self.recordAvg.append(ssc/blk)
        self.recordHigh.append(msc)
        
        saveFreq = 1000
        if (self.count) % saveFreq == 0:
            rawData = {
                'average' : self.recordAvg,
                'highest' : self.recordHigh
            }
            df = pd.DataFrame(rawData, columns = ['average', 'highest'])
            self.recordAvg = []
            self.recordHigh = []
            csvName = '../csv/' + str(int(self.count/saveFreq)) + '.csv'
            df.to_csv(csvName)
